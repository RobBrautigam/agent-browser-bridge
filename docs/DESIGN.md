...
| Write | `browser_navigate`, `browser_open_tab`, `browser_open_or_focus`, `browser_close_tab`, `browser_activate_tab`, `browser_click`, `browser_fill`, `browser_press_keys`, `browser_wait_for`, `browser_reload_extension` | Allowed, always audited. `browser_click` and `browser_press_keys` accept `trusted: true` to escalate to real input events through the debugger. `browser_open_or_focus` is the one tool written for a page a HUMAN will read: it reuses the tab already showing the address rather than adding a sixth copy of it. `browser_reload_extension` is the one tool that changes the BRIDGE rather than a page, and the one gated on something other than a tier (section 10). |
| Armed | `browser_eval_js` | Arbitrary JavaScript in an authenticated session: a full-compromise primitive, gated behind an explicit arm, from the Board or from the agent's own `bridge_arm` call (section 9, item 8). |
...
**`browser_open_or_focus` is the one operation that may name a local file.** It
OPENS or RELOADS only one whose path ends in `.html` or `.htm`. The `file:`
refusal exists because navigate (write) plus read (read) compose into an unarmed
local-file read primitive; a capability whose job is showing a generated page to
a person cannot do that job without local HTML, and restricting it to HTML
leaves no arbitrary file to aim a tab at. The rule is enforced in the extension
and again in the broker, because a rule that lives only in the extension is a
rule the broker merely trusts someone else to apply. SECURITY.md carries the two
further limits: Chromium's own file-URL injection opt-in, which nothing here
requests, and the origin-only audit log.

The same operation may **find** a tab already showing any other `file:` address
and **move** it, and may not open or reload one. That is a second, weaker
permission rather than a wider version of the first: moving a tab navigates
nothing and reads nothing, so it cannot be half of the navigate-then-read
composition, and a READ-tier tab list already reports every tab's address, so it
reveals nothing either. It exists so that a PDF gets the same one-page-one-tab
treatment as an HTML report, with the launcher doing the opening the bridge
refuses to do. Both sides compute the mode from the same rule
(`openOrFocusMode`), so neither delegates the decision to the other.

**The refusals are checked as schemes, not only as text prefixes.** A prefix
check is a rule about slashes, and `file:/C:/x` and `file:\\server\share\x` both
canonicalize to ordinary `file://` URLs without starting with `file://`. Before
0.4.0 both were accepted by `browser_navigate`, verified against a real browser;
see SECURITY.md.
...
| Another user account on the machine | Defended by a per-boot token in an owner-only file, proven in both directions at HELLO and never sent over the pipe (`shared/auth.mjs`). On Windows it can still take the pipe name while the broker is down, which keeps the broker from starting; no client sends it the token or trusts a frame from it. |
...
4. **The Windows named pipe has the default security descriptor**, readable by
   ...
   without a native addon, and native addons are banned. The name is also
   global: another account that creates it first, while the broker is down,
   keeps the broker from starting (it exits on `EADDRINUSE`, believing a broker
   is already up). Every client therefore makes the broker prove the token
   before it trusts a frame, and never sends the token itself, so a squatter
   gets neither the token nor the browser. What remains is a denial of service,
   which `doctor` reports as a pipe that answered without the broker's proof.
...
8. **The arm is agent-callable.** `bridge_arm` is an MCP tool, and the MCP
   role may originate `ARM` (`canOriginate` in `bridged/policy.mjs` admits
   every op in `BROKER_OPS`), so the broker cannot tell a Board click from an
   agent's call. The consent step in front of an agent's arm is the agent
   client's tool-approval prompt plus the tool's own instruction to ask; an
   operator who auto-approves every tool of this server has no human in front
   of `browser_eval_js`. Making the arm human-only (the Board, never the MCP
   role) would close that, and would break every workflow that arms from the
   agent today, so it waits for a release decision.

## 10. Three versions, and how a release reaches a browser

The hardest operational fact about this architecture is not in the topology
diagram: **a browser reads an unpacked extension's code once, when it loads
it.** Pulling a new version into the install folder changes nothing in a running
browser. So an install has three versions at any moment, and they drift apart on
purpose:

| Version | Changes when |
|---|---|
| the broker's | the broker restarts, which the supervisor does at login |
| the install folder's | somebody pulls |
| a profile's | that profile's extension is reloaded, per profile |

Until 0.4.0 none of the three was visible anywhere, and the consequence was
exactly what it sounds like: a release could be merged, deployed and believed,
while every browser on the machine still served the previous version. The
capability that shipped in 0.3.0 was proved working against a fake chrome and
answered `E_UNSUPPORTED` in the real one for hours.

Three pieces close it, and they are deliberately separate:

1. **The extension reports its manifest version on connect.** It always did, in
   the REGISTER envelope, and the broker always stored it on the route. Nothing
   rendered it. Now `browser_list_profiles`, `bridge_status`, the board and the
   popup all do.
2. **The broker re-reads the folder's manifest** rather than reporting its own
   version, because the folder changes under a broker that has been up since
   login, and the difference between the two is the signal.
3. **`reloadExtension` asks one profile's extension to reload itself**, gated on
   the folder holding a different version from the profile. The gate is in the
   broker because only the broker can see both numbers. `scripts/reload-extension.mjs`
   is the shell-callable form, because the natural caller is `git pull`.

The bootstrap problem is real and has no software fix: the release that ADDS the
self-reload cannot use it, because the code that would serve the request is the
code being replaced. So upgrading from 0.3.0 or earlier costs one click per
profile, and every release after that costs none.
...
scripts/      sync-config, rename, keygen, install-host, install-broker,
              ...
              claim, label, open-or-focus, reload-extension
